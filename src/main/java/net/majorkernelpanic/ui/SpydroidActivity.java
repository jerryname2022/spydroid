...
import net.majorkernelpanic.api.CustomHttpServer;
import net.majorkernelpanic.api.CustomRtspServer;
...
/**
 * Spydroid basically launches an RTSP server and an HTTP server, clients can
 * then connect to them and start/stop audio/video streams on the phone.
 */
public class SpydroidActivity extends FragmentActivity {
	...
	public void onStart() {
		...
		bindService(new Intent(this, CustomHttpServer.class),
				mHttpServiceConnection, Context.BIND_AUTO_CREATE);
		bindService(new Intent(this, CustomRtspServer.class),
				mRtspServiceConnection, Context.BIND_AUTO_CREATE);
	...
	private ServiceConnection mHttpServiceConnection = new ServiceConnection() {
		...
		@Override
		public void onServiceConnected(ComponentName name, IBinder service) {
			mHttpServer = (CustomHttpServer) ((TinyHttpServer.LocalBinder) service)
					.getService();
	...
	private TinyHttpServer.CallbackListener mHttpCallbackListener = new TinyHttpServer.CallbackListener() {
		...
		public void onError(TinyHttpServer server, Exception e, int error) {
			...
			if (error == TinyHttpServer.ERROR_HTTP_BIND_FAILED
				...
				String str = error == TinyHttpServer.ERROR_HTTP_BIND_FAILED ? "HTTP"
						: "HTTPS";
				new AlertDialog.Builder(SpydroidActivity.this)
						.setTitle(R.string.port_used)
	...
	class SectionsPagerAdapter extends FragmentPagerAdapter {
		...
		public HandsetFragment getHandsetFragment() {
			if (device == HANDSET) {
				return (HandsetFragment) getSupportFragmentManager()
						.findFragmentByTag(
								"android:switcher:" + R.id.handset_pager + ":0");
			} else {
				return (HandsetFragment) getSupportFragmentManager()
						.findFragmentById(R.id.handset);
			}
		}

		public PreviewFragment getPreviewFragment() {
			if (device == HANDSET) {
				return (PreviewFragment) getSupportFragmentManager()
						.findFragmentByTag(
								"android:switcher:" + R.id.handset_pager + ":1");
			} else {
				return (PreviewFragment) getSupportFragmentManager()
